...
var EMAIL_SERVICE_PROVIDERS = {
  GOOGLE: {
    RE_FOOTER_TEXT: /\n?[-\s]*\s*\n?You received this message because you are subscribed to the Google Groups([\s\S]*)$/im
  }
};
...
function extractSignature (msgBody) {
  try {
    ...
    _.forEach(EMAIL_SERVICE_PROVIDERS, function(emailServiceProvider) {
      strippedBody = strippedBody.replace(emailServiceProvider.RE_FOOTER_TEXT,
          '');
    });
